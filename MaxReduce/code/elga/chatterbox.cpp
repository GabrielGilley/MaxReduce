...
ZMQChatterbox::ZMQChatterbox(const ZMQAddress &addr) :
        sock_rep_(NULL), sock_pub_(NULL), sock_sub_(NULL), sock_pull_(NULL), last_heartbeat_(), addr_(addr) {
...
void ZMQChatterbox::send(zmq_socket_t sock, const char *data, size_t size, bool nowait) {
    ...
    if (ret < 0) { std::cerr << "errno=" << errno << std::endl; perror("ZMQChatterbox::send"); throw std::runtime_error("Unable to send"); }
    #ifdef DEBUG_VERBOSE
