...
TEST(build_out_edges) {
    ...
    {
        DBEntry<> e;
        e.clear().add_tag("BTC", "tx").value() = "{\"tx\":{\"txid\": \"B\", \"vout\": [{\"value\": 6.0, \"n\": 0}]}}";
        db.add_entry(move(e));
    }

    // Create the tx IN edges
    {
        DBEntry<> e; e.clear().add_tag("BTC", "tx-in-edge", "from=A", "n=0").value() = "B";      // A->B
        dbkey_t key {pack_chain_info(BTC_KEY, TX_IN_EDGE_KEY, 0), 10, 0};
        e.set_key(key);
        db.add_entry(move(e));
    }
    {
        DBEntry<> e; e.clear().add_tag("BTC", "tx-in-edge", "from=A", "n=1").value() = "C";      // A->C
        dbkey_t key {pack_chain_info(BTC_KEY, TX_IN_EDGE_KEY, 0), 10, 1};
        e.set_key(key);
        db.add_entry(move(e));
    }
    {
        DBEntry<> e; e.clear().add_tag("BTC", "tx-in-edge", "from=B", "n=0").value() = "C";      // B->C
        dbkey_t key {pack_chain_info(BTC_KEY, TX_IN_EDGE_KEY, 0), 11, 0};
        e.set_key(key);
        db.add_entry(move(e));
    }
    ...
    for (auto & [key, entry] : db.entries()) {
        ...
        } else if (entry.has_tag("tx") && entry.has_tag("BTC_tx_out_edges:inactive")) {
            inactive_count++;
        }
    }
    EQ(e_01, true);
...
TEST(bad_json) {
    ...
    for (auto& [key, entry] : db.entries() ) {
        if (entry.has_tag("BTC_tx_out_edges:done")) TEST_FAIL;
        if (entry.has_tag("BTC_tx_out_edges:fail")) success_count++;
    }
...
TEST(in_edge_nodata) {
    ...
    { DBEntry<> e; e.clear().add_tag("BTC", "txid", "B").value() = "1"; db.add_entry(e); }
    { DBEntry<> e; e.clear().add_tag("BTC", "txid", "C").value() = "2"; db.add_entry(e); }
    { DBEntry<> e; e.clear().add_tag("BTC", "txid", "D").value() = "3"; db.add_entry(e); }

    // Create the transaction JSON
    {
        DBEntry<> e;
        e.clear().add_tag("BTC", "tx").value() = "{\"tx\":{\"txid\": \"B\", \"vout\": [{\"value\": 7.0, \"n\": 0}, {\"value\": 8.0, \"n\": 1}],"
                "\"vin\": [{\"txid\": \"A\", \"vout\": 0 }]}}";
        db.add_entry(e);
    }
    {
        DBEntry<> e;
        e.clear().add_tag("BTC", "tx").value() = "{\"tx\":{\"txid\": \"C\", \"vout\": [{\"value\": 6.0, \"n\": 0}],"
                "\"vin\": [{\"txid\": \"A\", \"vout\": 1}]}}";
        db.add_entry(e);
    }
    {
        DBEntry<> e;
        e.clear().add_tag("BTC", "tx").value() = "{\"tx\":{\"txid\": \"D\", \"vout\": [{\"value\": 7.0, \"n\": 0}],"
                "\"vin\": [{\"txid\": \"B\", \"vout\": 0 },"
                "{\"txid\": \"C\", \"vout\": 0}]}}";
        db.add_entry(e);
    }
...
TEST(check_keys) {
    ...
    {
        DBEntry<> e; //A
        e.clear().add_tag("BTC", "tx").value() = "{\"tx\":{\"txid\": \"8c14f0db3df150123e6f3dbbf30f8b955a8249b62ac1d1ff16284aefa3d06d87\", "
            "\"vout\": [{\"value\": 10.0, \"n\": 0}, {\"value\": 9.0, \"n\": 1}, {\"value\": 5.0, \"n\": 2}, {\"value\": 1.2345,\"n\": 3}]}}";
        db.add_entry(move(e));
    }
    {
        DBEntry<> e; //B
        e.clear().add_tag("BTC", "tx").value() = "{\"tx\":{\"txid\": \"fff2525b8931402dd09222c50775608f75787bd2b87e56995a7bdd30f79702c4\", "
            "\"vout\": [{\"value\": 6.0, \"n\": 0}]}}";
        db.add_entry(move(e));
    }
    ...
    {
        // A->B
        DBEntry<> e; e.clear().add_tag("BTC", "tx-in-edge", "from=8c14f0db3df150123e6f3dbbf30f8b955a8249b62ac1d1ff16284aefa3d06d87", "n=0");
    ...
    {
        // A->C
        DBEntry<> e; e.clear().add_tag("BTC", "tx-in-edge", "from=8c14f0db3df150123e6f3dbbf30f8b955a8249b62ac1d1ff16284aefa3d06d87", "n=1");
    ...
    {
        // B->C
        DBEntry<> e; e.clear().add_tag("BTC", "tx-in-edge", "from=fff2525b8931402dd09222c50775608f75787bd2b87e56995a7bdd30f79702c4", "n=0");
        e.value() = "87a157f3fd88ac7907c05fc55e271dc4acdc5605d187d646604ca8c0e9382e03";
