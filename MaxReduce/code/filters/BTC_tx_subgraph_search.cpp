...
#define FILTER_NAME "BTC_tx_subgraph_search"
...
class BFS : public GASFilter<T,T> {
    ...
    protected:
        ...
        T init(DBAccess* acc) {
            //TODO for now, expect an entry with key {BTC_KEY,
            ...
            chain_info_t key_a = pack_chain_info(BTC_KEY, BFS_INITIAL_VALUE, 0);
    ...
    public:
        ...
        BFS(DBAccess* acc) : GASFilter<T,T>::GASFilter(acc) {
            chain_info_t chain_info = pack_chain_info(BTC_KEY, DISTANCE_KEY, 0);
...
extern "C" {
    ...
    extern bool should_run(const DBAccess* access) {
        ...
        chain_info_t btc_ci_not_utxo = pack_chain_info(BTC_KEY, TX_OUT_EDGE_KEY, NOT_UTXO_KEY);
        //chain_info_t btc_ci_utxo = pack_chain_info(BTC_KEY, TX_OUT_EDGE_KEY, UTXO_KEY);
