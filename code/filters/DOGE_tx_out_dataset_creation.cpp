...
#define FILTER_NAME "DOGE_tx_out_dataset_creation"
...
void run_(const DBAccess *access) {
    ...
    const char* new_tags_addr_to_output[] = {"DOGE", "address_to_tx_outputs.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_output[] = {"DOGE", "tx_outputs.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_tx_to_output[] = {"DOGE", "tx_to_outputs.csv", "CROSSCHAIN_DATASET", ""};
    ...
    for (auto & vout: vouts.GetArray()) {
        ...
        chain_info_t ci = pack_chain_info(DOGE_KEY, TX_IN_EDGE_KEY, 0);
