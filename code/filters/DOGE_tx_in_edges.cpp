#include "BTC_BASED_tx_in_edges.hpp"
...
void run_(const DBAccess *access)  {
    return btc_based_tx_in_edges(access, crypto_tag, filter_name, filter_done_tag, filter_fail_tag);
}
...
extern "C" {
    ...
    extern bool should_run(const DBAccess* access) {
        return should_run_btc_based_tx_in_edges(access, crypto_tag, filter_done_tag, filter_fail_tag);
    }
