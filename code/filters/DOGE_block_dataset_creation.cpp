...
#define FILTER_NAME "DOGE_block_dataset_creation"
...
void run_(const DBAccess *access) {
    ...
    const char* new_tags_addr[] = {"DOGE", "address.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_block[] = {"DOGE", "blocks.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_b_to_bc[] = {"DOGE", "block_to_blockchain.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_tx[] = {"DOGE", "tx.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_tx_to_b[] = {"DOGE", "tx_to_block.csv", "CROSSCHAIN_DATASET", ""};
    ...
    string new_val_b_to_bc = block_hash + "," + "DOGE";
    ...
    for (auto & tx : txs.GetArray()) {
        ...
        for (auto & vout: vouts.GetArray()) {
            ...
            for (auto & addr : addresses.GetArray()) {
                ...
                chain_info_t addr_ci = pack_chain_info(DOGE_KEY, ADDR_KEY, 0);
...
extern "C" {
    ...
    extern bool should_run(const DBAccess* access) {
        ...
        //make sure the entry includes "DOGE" and "block" but not
        //"DOGE_block_to_tx:done" or "DOGE_block_to_tx:fail"
        while ((*tags)[0] != '\0') {
            if (string(*tags) == "DOGE")
                found_btc = true;
