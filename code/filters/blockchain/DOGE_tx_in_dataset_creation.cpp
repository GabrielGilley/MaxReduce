...
#define FILTER_NAME "DOGE_tx_in_dataset_creation"
...
void run_(const DBAccess *access) {
    ...
    const char* new_tags_addr_to_input[] = {"DOGE", "address_to_tx_inputs.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_input[] = {"DOGE", "tx_inputs.csv", "CROSSCHAIN_DATASET", ""};
    const char* new_tags_tx_to_input[] = {"DOGE", "tx_to_inputs.csv", "CROSSCHAIN_DATASET", ""};
    ...
    if (is_coinbase(access)) {
        ...
        chain_info_t search_ci = pack_chain_info(DOGE_KEY, TX_KEY, 0);
        ...
        chain_info_t search_ci = pack_chain_info(DOGE_KEY, OUT_VAL_KEY, access->key.c);
    ...
    // expect an entry with key {{DOGE_KEY, VOUT_ADDR_KEY, 0}, txid, n} and value is the address (if there is one -- if not, the entry should not exist)
    // lookup the address of this input (if there is one)
    chain_info_t search_addr_ci = pack_chain_info(DOGE_KEY, VOUT_ADDR_KEY, 0);
