#include "BTC_BASED_block_to_tx.hpp"
...
void run_(const DBAccess *access) {
    return btc_based_block_to_tx(access, crypto_tag, filter_name, filter_fail_tag, filter_done_tag);
}
...
extern "C" {
    ...
    extern bool should_run(const DBAccess* access) {
        return should_run_btc_based_block_to_tx(access, crypto_tag, filter_fail_tag, filter_done_tag);
    }
