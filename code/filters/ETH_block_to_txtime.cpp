#include "ETH_BASED_block_to_txtime.hpp"
...
void run_(const DBAccess *access) {
    return eth_based_block_to_txtime(access, crypto_tag, filter_fail_tag, filter_done_tag);
}
...
extern "C" {
    ...
    extern bool should_run(const DBAccess* access) {
        return should_run_eth_based_block_to_txtime(access, crypto_tag, filter_done_tag, filter_fail_tag);
    }
