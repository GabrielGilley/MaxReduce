...
#define FILTER_NAME "test_algdb_simple_group"
static const char* filter_name = FILTER_NAME;

void run_(const DBAccess *access) {

    //void g_TEST_simple_group_test_run([[maybe_unused]] void* access_raw) {
    //DBAccess* access = (DBAccess*)access_raw;

    // Note: this will run on the "group" of 1,1,5 and the group of 1,5,0, not
    // on those entries

    auto &out_msgs = *(access->group->out_msgs);
    auto &in_msgs = *(access->group->in_msgs);

    // 'A' = (vertex 1) 1,1,5 ; 'B' = (vertex 5) 1,5,0
    // If iter == 0:
    //      A: send msg to B
