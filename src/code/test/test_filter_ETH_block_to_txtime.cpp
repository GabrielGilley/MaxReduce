...
TEST(tag_added) {
    ...
    for (auto & [key, entry] : db.entries())
        EQ(entry.has_tag("ETH_block_to_txtime:done"), false);
    ...
    for (auto & [key, entry] : db.entries()) {
        if (entry.has_tag("block")) {
            EQ(entry.has_tag("ETH_block_to_txtime:done"), true);
            count++;
        }
    }
    EQ(count, SIMPLE_ETH_NUM_BLOCKS);
...
TEST(tag_not_added) {
    ...
    EQ(db.size(), SIMPLE_ETH_NUM_BLOCKS + 1);
...
TEST(exclude_run) {
    ...
    EQ(db.size(), SIMPLE_ETH_NUM_BLOCKS + SIMPLE_ETH_NUM_TX);
    ...
    DBEntry<> e;
    e.add_tag("ETH").add_tag("block").add_to_value("{\"transactions\":[{\"hash\":\"0x12345678\"}],\"timestamp\":\"0x56bfb415\"}");
    db.add_entry(e);

    EQ(db.size(), SIMPLE_ETH_NUM_BLOCKS + SIMPLE_ETH_NUM_TX + 1);

    db.process();

    EQ(db.size(), SIMPLE_ETH_NUM_BLOCKS + SIMPLE_ETH_NUM_TX + 2);

    for (auto & [key, entry] : db.entries()) {
        EQ(entry.has_tag("ETH_block_to_txtime:fail"), false);
    }

    e.add_tag("ETH_block_to_txtime:fail");
    db.add_entry(e);

    EQ(db.size(), SIMPLE_ETH_NUM_BLOCKS + SIMPLE_ETH_NUM_TX + 3);
...
TEST(bad_json) {
    ...
    for (auto & [key, entry] : db.entries()) {
        if (entry.has_tag("invalid-json")) {
            EQ(entry.has_tag("ETH_block_to_txtime:done"), false);
            EQ(entry.has_tag("ETH_block_to_txtime:fail"), true);
