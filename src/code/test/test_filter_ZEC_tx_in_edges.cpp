...
TEST(build_in_edges) {
    ...
    e.clear().add_tag("ZEC", "tx").value() = "{\"tx\":{\"txid\": \"A\", \"vin\": [{\"coinbase\": \"12345\" }]}}";
    db.add_entry(e);
    e.clear().add_tag("ZEC", "tx").value() = "{\"tx\":{\"txid\": \"B\", \"vin\": [{\"txid\": \"A\", \"vout\": 0}]}}";
    db.add_entry(e);
    e.clear().add_tag("ZEC", "tx").value() = "{\"tx\":{\"txid\": \"C\", \"vin\": [{\"txid\": \"A\", \"vout\": 1}, {\"txid\": \"B\", \"vout\": 0}]}}";
    db.add_entry(e);
...
TEST(coinbase) {
    ...
    for (auto & [key, entry] : db.entries()) {
        if (entry.has_tag("tx-in-edge") && entry.has_tag("from=COINBASE"))
            success = true;
        if (entry.has_tag("ZEC_tx_in_edges:fail")) {
            TEST_FAIL
        }
    }
...
TEST(check_keys) {
    ...
    // Create the transaction JSON
    {DBEntry<> e;
    e.clear().add_tag("ZEC", "tx").value() = "{\"tx\":{\"txid\": \"" + a_key + "\", \"vin\": [{\"coinbase\": \"12345\" }]}}";
    db.add_entry(e);}
    {DBEntry<> e;
    e.clear().add_tag("ZEC", "tx").value() = "{\"tx\":{\"txid\": \"" + b_key + "\", \"vin\": [{\"txid\": \"" + a_key + "\", \"vout\": 0}]}}";
    db.add_entry(e);}
    {DBEntry<> e;
    e.clear().add_tag("ZEC", "tx").value() = "{\"tx\":{\"txid\": \"" + c_key + "\", \"vin\": [{\"txid\": \"" + a_key + "\", \"vout\": 1}, {\"txid\": \"" + b_key + "\", \"vout\": 0}]}}";
    db.add_entry(e);}
    ...
    chain_info_t expected_ci = pack_chain_info(ZEC_KEY, TX_IN_EDGE_KEY, 0);
