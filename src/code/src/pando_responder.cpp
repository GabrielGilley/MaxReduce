...
int main_(int argc, char **argv) {
    ...
    if (argc != 3) {
        cerr << "Usage: pando_responder bind-addr seed-addr\n"
            "\n"
            "Parameters:\n"
    ...
    if (seed_addr.serialize() == bind_addr.serialize()) throw runtime_error("Unable to connect responder to itself");

    cerr << "[Pando] [DEBUG] Bind addr=" << bind_addr.get_conn_str(bind_addr, REQUEST) << endl;
    ParDBThread<PandoResponder> db { bind_addr };
