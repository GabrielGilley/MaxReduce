...
namespace elga {
    ...
    class ZMQAddress {
        private:
            ...
            /** Pre-compute and store the address-format addr */
        ...
        public:
            ...
            ZMQAddress(uint32_t addr, localnum_t localnum) : addr_(addr), localnum_(localnum) {
                precompute_strings_();
            }
            ...
            /** Support the prefix ++ operator by incrementing localnum by 1 */
            ZMQAddress operator+(size_t offset) {
                ZMQAddress ret { this->get_addr(), (localnum_t)(this->get_localnum() + offset) };
                return ret;
            }

            /** Support the prefix -- operator by decrementing localnum by 1 */
            ZMQAddress operator-(size_t offset) {
                if (offset > this->get_localnum())
                    throw std::runtime_error("Cannot decrement address");
                ZMQAddress ret { this->get_addr(), (localnum_t)(this->get_localnum() - offset) };
                return ret;
            }
            ...
            const std::string get_addr_str() const { return addr_str_; }
